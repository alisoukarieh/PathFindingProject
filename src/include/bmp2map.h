...
/*!F
	...
	Ce format est relativement facile, mais va quand mÕme faire peur aux novices du domaine.

	Alors regardons le main, ainsi que le .h : ce sont les mÕmes que l'exemple pr»c»dent sur les PPM.
	Je vous laisse regarder l'exemple pr»c»dent.

	De mÕme toutes les fonctions, sauf chager et sauver sont les mÕmes.
...
http://www.commentcamarche.net/contents/video/format-bmp.php3
	...
	Un BMP, c'est un header (des donn»es), et ensuite le pixels les uns á la suite des autres, non compress»s.
	...
	Dans notre cas, ce sera toujours á 54, aprÀs la taille du header.

	Vous voyez que j'ai fait des structures qui correspondent á cela dans bmp.c
	...
	Nous avons ensuite une sous structure EntÕte de l'image (bien expliqu»e dans le lien ci dessus). Nous avons :
    - sa taille (40)
    - la largeur et la hauteur de l'image (important !) regardez dans charger et sauver, je les associe á I->w et I->h
    - le nombre de plans (toujours á 1 : des extentions du format BMP qui n'ont jamais »t» exploit»es)
      j'imagine qu'il »tait pr»vu de faire des fichiers BMP a plusieurs plans, plusieurs images dans le mÕme fichier ?
...
la mÕme couleur seront donc chang»s en cons»quence !
...
Passons maintenant á l'image elle mÕme !

Les pixels sont cod»s les uns á la suite des autres.
On pourrait donc se dire "un grand fread et c'est pli» !"

Cependant, h»las, nous ne stockons pas, ma structure et les BMP, les choses de la mÕme fa¡on.

Ma structure est ainsi faite :
1) Je code les pixels de gauche á droite, puis de haut en bas (sens de lecture occidental)
...
1) stockage des pixels de gauche á droite, mais de ---> BAS EN HAUT <---

Donc vous remarquerez dans ma fonction charger et sauver, les pixels sont lus/»crits non pas á position i,j
par rapport á ma structure, mais i,I->h-j-1, ce qui permet de bien mettre les pixels ou il faut.
...
Le reste de la division de 3*X par 4 me dit combien d'octets sont en plus. Et en cons»quence, je complÀte par des 0
J'appelle pitch le nombre d'octets á rajouter ici.
...
Voila, il ne me reste plus qu'á »crire ou lire l'image...

Ensuite, je manipule de la mÕme maniÀre que dans l'exemple pr»c»dent.
